      ...
      if (inputSignup.email.length > 0) {
        ...
        try {
          const response = await axios.post(
            `${REACT_APP_deploy_server_URL}/is-email-available`,
            [inputSignup.email]
          );
...
//<-------------------------Regex Section ----------------------------------->  
  ...
  const handleSignupSubmit = async (e) => {
    ...
    if ( (username_pattern.test(inputSignup.username)&& password_pattern.test(inputSignup.password)) && isEmailAvailable === true && isLoading.isEmailAvailable === false) {
      ...
      try {
        e.preventDefault()
        const res = await axios.post(`${REACT_APP_deploy_server_URL}/todo_app/signup`,
          inputSignup
        );
...
//<-------------------------Start Login Section ----------------------------------->
        ...
        const handleCallbackResponse = async (response) => { 
          ...
          try {
            ...
            const res = await axios.post(
              `${REACT_APP_deploy_server_URL}/todo_app/login_google`,
  ...
  const handleLoginSubmit = async (e) => {
    ...
    try {
      setLoading(true)
      e.preventDefault()
      const res = await axios.post(`${REACT_APP_deploy_server_URL}/todo_app/login`,inputLogin, {withCredentials: true}
      );
